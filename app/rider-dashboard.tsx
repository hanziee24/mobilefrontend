        ...
        <View style={styles.card}>
          ...
          {!loading && deliveries.length > 0 && (
            <Text style={styles.cardSubtitle}>
              {deliveries.length} {deliveries.length === 1 ? 'delivery' : 'deliveries'} ready to manage
            </Text>
          )}
          {loading ? (
            ...
            deliveries.map((delivery) => (
              <ActiveDeliveryCard
                ...
                pickupDisabled={!isOnline}
                onPickupDisabled={() => Alert.alert('Offline', 'You must be online to pick up packages')}
                onPickup={() => {
                ...
                onNavigate={() => router.push('/rider-navigation')}
                onAdvance={() => router.push({ pathname: '/update-status', params: { id: delivery.id } } as any)}
                onComplete={() => handleCompleteFlow(delivery.id)}
                onQr={() => router.push({ pathname: '/delivery-qr', params: { id: delivery.id } } as any)}
                onChat={() => router.push({ pathname: '/chat', params: { deliveryId: delivery.id, trackingNumber: delivery.tracking_number } } as any)}
              />
            ))
          )}
        </View>
